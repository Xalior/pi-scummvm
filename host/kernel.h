//
// kernel.h — the Circle kernel that hosts ScummVM.
//
// Device ownership: this kernel brings up interrupts, the timer, a serial
// console carrying stdio, the SD card (FatFs, holding the game data and the
// configuration file), the USB host controller and the cooperative
// scheduler. Video and audio belong to circle-libsdl2 and are created inside
// SDL_Init when ScummVM calls it. So do the CPU clock and the case fan,
...
//   core 1   the APPLICATION core. ScummVM, alone: the interpreter, the
//            mixer and the user interface. Every platform call it makes is
...
private:
    ...
    // service once the card is up, so FatFs — and through it every file
    // ScummVM opens — reaches the card through it without anything above
