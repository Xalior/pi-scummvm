...
// ---- what newlib does not have --------------------------------------------

// access() is POSIX and newlib's Circle glue does not provide it. ScummVM
// asks it three questions about a path — does it exist, may I read it, may I
// write it — and on this machine the only one with a real answer is the
// first: the card is one FAT filesystem with no permissions on it, so
// anything that exists can be both read and written.
//
// It is written in terms of stat(), which is already wrapped above and so
// already reaches the core that owns the card.
